        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-12 items-center">
          <div className="order-2 md:order-1">
            ...
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-4 md:mb-6 text-dental-800">
              {title.split(' ').slice(0, -1).join(' ')} <span className="text-dental-600">{title.split(' ').slice(-1)}</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row flex-wrap gap-3 md:gap-4">
              {features.map((feature, index) => (
                <div key={index} className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-full shadow-sm">
            ...
            <div className="mt-6 md:mt-8 flex flex-col sm:flex-row gap-4">
              <Button asChild>
                <Link href={primaryButtonLink} className="flex items-center justify-center">
                  {primaryButtonText} <ChevronRight className="ml-2 h-4 w-4" />
                </Link>
              </Button>
              <Button asChild variant="outline" className="border-dental-200 text-dental-700 hover:bg-dental-50">
